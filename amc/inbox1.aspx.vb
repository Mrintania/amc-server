...
Partial Class inbox
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Request.QueryString("para1") = "" Then
            ...
            strsql = "Select * From DataNews Where (From2 LIKE '%" & Trim(labname.Text) & "%') AND (Status = 1) ORDER BY Id DESC "
        ...
        Else
            ...
            strsql = strsql & " (From2 LIKE '%" & Trim(labname.Text) & "%')  AND (Add1 LIKE '%" & Request.QueryString("para1") & "%') AND (Status = 1) OR"
            strsql = strsql & " (From2 LIKE '%" & Trim(labname.Text) & "%')  AND (Add1 LIKE '%" & Request.QueryString("para1") & "%') AND (Status = 1 "
            strsql = strsql & " ORDER BY Id DESC"
    ...
    Public Function count_read(ByVal X As Integer) As Integer
        ...
        If X = 1 Then
            strsql = "select COUNT(*) from DataNews  WHERE (From2 LIKE '%" & Trim(labname.Text) & "%')AND (Status = 1) "
        End If
        If X = 2 Then
    ...
    Protected Sub cmdfine_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdfine.Click
        Response.Redirect("inbox1.aspx?para1=" & Trim(txtfind.Text) & "&nid=" & Request.QueryString("nid"))
    End Sub
    ...
    Public Function ResiveGet(ByVal X As String) As String
        ...
        For i = 0 To count1 - 1
            If i = 0 Then
                strcount = Trim(ds.Tables("ConfirmTable").Rows(i).Item("ConfirmStation"))
            Else
                strcount += " , " + Trim(ds.Tables("ConfirmTable").Rows(i).Item("ConfirmStation"))
            End If
    ...
    Public Function datesendGet(ByVal X As Integer) As String
        ...
        If ds.Tables("DataNews").Rows.Count <> 0 Then
            strpic = Convert.ToString(ds.Tables("DataNews").Rows(0).Item("Datenow"))

            'If (Not IsDBNull(ds.Tables("DataNews").Rows(0).Item("Datenow"))) Then
